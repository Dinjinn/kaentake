...
public:
    ...
    void LoadSprites() {
        ...
        m_pBtSort[0]   = LoadSprite(L"UI/UIWindow.img/Bag/BtSort/normal/0");
        m_pBtSort[1]   = LoadSprite(L"UI/UIWindow.img/Bag/BtSort/pressed/0");
        m_pBtSort[2]   = LoadSprite(L"UI/UIWindow.img/Bag/BtSort/mouseOver/0");
    ...
    bool MatchItem(int id) const {
        ...
        char ids[16]; _snprintf_s(ids, sizeof(ids), "%d", id); ids[15] = 0;
...
void CUIBagWindow::Draw(const RECT* pRect) {
    ...
    for (int v = 0; v < kInvVisCount; ++v) {
        ...
        if (slot >= 0 && slot < kInvCap && s.ready && s.id[slot]) {
            ...
            int q = s.qty[slot];
            if (q >= 1) {
                char num[12]; _snprintf_s(num, sizeof(num), "%d", q); num[11] = 0;
    ...
    if (pfDk) {
        ...
        if (Filtering()) {
            char cnt[16]; _snprintf_s(cnt, sizeof(cnt), "%d", m_displayCount); cnt[15] = 0;
...
static void CreateInvButton(void* pParentWnd) {
    s_invBtn = ZRef<CCtrlButton>();     // release the previous (window-destroyed) button
