...
bool ReadPixelsLocked(IWzCanvas* pCanvas, int w, int h, uint32_t* out) {
    ...
    __try {
        if (FAILED(pCanvas->get_pixelFormat(&fmt)))
            fmt = CP_UNKNOWN;
    ...
    __try {
        if (FAILED(pCanvas->get_rawCanvas(0, 0, &raw)))
            raw = nullptr;
    } __except (EXCEPTION_EXECUTE_HANDLER) {
        raw = nullptr;
    }
    ...
    __try {
        ...
        if (SUCCEEDED(raw->raw__LockAddress(&pitch, &vAddr))) {
            ...
            if (base_vt == VT_I4 || base_vt == VT_UI4 || base_vt == VT_INT || base_vt == VT_UINT) {
            ...
            for (int y = 0; y < h; ++y) {
                ...
                if (bpp == 4) {
                    ...
                    for (int x = 0; x < w; ++x) {
                        ...
                        dstRow[x] = ((((s >> 12) & 0xFu) * 17u) << 24) | ((((s >> 8) & 0xFu) * 17u) << 16) | ((((s >> 4) & 0xFu) * 17u) << 8) | (((s & 0xFu) * 17u));
                    }
                }
            }
            ok = true;
    ...
    if (base) {
        ...
        __try {
            raw->raw__UnlockAddress(nullptr);
        } __except (EXCEPTION_EXECUTE_HANDLER) {
        }
    }
    __try {
...
void ReadPixelsSlow(IWzCanvas* pCanvas, int w, int h, uint32_t* out) {
    __try {
        for (int y = 0; y < h; ++y) {
            for (int x = 0; x < w; ++x) {
                uint32_t px = 0;
                if (FAILED(pCanvas->get_pixel(x, y, &px)))
                    px = 0;
        ...
        for (int y = 0; y < h; ++y) {
            ...
            while (x < w) {
                ...
                if ((c >> 24) == 0) {
                    ++x;
                    continue;
                } // transparent: leave it
                int end = x + 1;
                while (end < w && row[end] == c)
                    ++end;
...
// Raw pointer of every clone built above, so a walk can tell "this canvas is
// already one of mine". A UOL under one posture can resolve THROUGH a sibling
// posture that was swapped moments earlier in the same build; tinting that
// resolved canvas again would stack the transform (2x hue = a different colour
// on exactly the frames that share art). Cleared with the cache, whose refs are
// what keep these pointers alive.
std::unordered_set<IWzCanvas*> g_clonePtrs;

bool IsOwnClone(IWzCanvas* p) {
    return p && g_clonePtrs.count(p) != 0;
}
...
IWzCanvasPtr GetTinted(IWzCanvasPtr pSrc, const WeaponTint& t) {
    ...
    if (clone)
        g_clonePtrs.insert(static_cast<IWzCanvas*>(clone));
    return clone;
}
    ...
    for (const std::wstring& name : ChildNames(pNode)) {
        ...
        if (isEffect) {
            ...
            if (SUCCEEDED(pUnk.QueryInterface(__uuidof(IWzCanvas), &pOrig)) && pOrig) {
                // A UOL that resolved through an already-swapped sibling posture:
                // its home node carries the tint, so touching it again would stack it.
                if (IsOwnClone(static_cast<IWzCanvas*>(pOrig)))
                    continue;
        ...
        IWzPropertyPtr pSub;
        if (FAILED(pUnk.QueryInterface(__uuidof(IWzProperty), &pSub)) || !pSub)
            continue;
        if (isEffect)
            SwapSubtree(pSub, t, depth + 1, Layer::Effects);
        else
            SwapEffectSubtrees(pSub, t, depth + 1);
    }
}
...
const wchar_t* CharacterSubdirOf(int itemId) {
    ...
    case 102:
    case 103:
        return L"Accessory"; // face / eye / earring
    case 104:
        return L"Coat";
    case 105:
        return L"Longcoat";
...
void SwapLayerOf(IWzPropertyPtr p, const WeaponTint& t, Layer layer) {
    if (layer == Layer::Effects)
        SwapEffectSubtrees(p, t, 0);
    else
        SwapSubtree(p, t, 0, Layer::Body);
}
...
void SwapInTintFor(int itemId, const wchar_t* /*actionName*/, const WeaponTint& t, Layer layer) {
    ...
    try {
        ...
        // THE WHOLE IMG, not the one posture being built. Postures share frames
        // through UOLs into sibling actions (stand1/2/weapon -> ../../alert/0/weapon),
        // and a UOL whose target sits outside the swapped subtree resolves to the
        // ORIGINAL canvas at build time: one animation frame keeps the old colour and
        // the item flickers once per cycle. Walking from the img root tints every
        // canvas at its home location, so a UOL lands on a swapped node no matter
        // which posture it crosses into -- the same hole the item-effect walk already
        // closed, affordable for the same reason: GetTinted caches every clone, and
        // UOL/inlink sharing keeps the distinct canvases far fewer than the nodes.
        for (const std::wstring& name : ChildNames(pImg)) {
            if (name == L"info")
                continue;
            IWzPropertyPtr p = pImg->item[name.c_str()].GetUnknown();
            if (p)
                SwapLayerOf(p, t, layer);
        }
    } catch (...) {
...
bool ReadCanvasPixels(IWzCanvasPtr pSrc, int& w, int& h, std::vector<uint32_t>& out) {
    ...
    try {
        ...
        if (!ReadPixelsLocked(tmp, w, h, out.data()))
            ReadPixelsSlow(tmp, w, h, out.data());
    } catch (...) {
        return false;
    }
    return true;
...
IWzCanvasPtr CanvasAtPath(IWzPropertyPtr pImg, const wchar_t* path) {
    ...
    try {
        ...
        IWzPropertyPtr pParent = (cut == std::wstring::npos)
                                         ? pImg
                                         : PropertyAtPath(pImg, full.substr(0, cut));
        if (!pParent)
            return nullptr;
        ...
        if (!pUnk)
            return nullptr;
        IWzCanvasPtr cv;
        if (FAILED(pUnk.QueryInterface(__uuidof(IWzCanvas), &cv)))
            return nullptr;
        return cv;
    } catch (...) {
        return nullptr;
...
const std::vector<FaceIrisPlan>& FacePlansFor(int faceId) {
    ...
    for (int r = 0; r < 3; ++r)
        pRefImg[r] = FaceImg(base + kFaceRefOffsets[r]);
    ...
    if (pWornImg && pRefImg[0] && pRefImg[1] && pRefImg[2]) {
        for (size_t i = 0; i < _countof(kFaceIrisPaths); ++i) {
            ...
            if (!ReadCanvasPixels(CanvasAtPath(pWornImg, path), w, h, worn))
                continue;
            ...
            if (!hits)
                continue; // nothing varies: not an iris canvas
            plans[i].w = w;
            plans[i].h = h;
    ...
    bool anyPlan = false;
    for (const FaceIrisPlan& p : plans)
        anyPlan = anyPlan || !p.Empty();
    if (!anyPlan) {
...
IWzCanvasPtr CloneIrisTinted(IWzCanvasPtr pSrc, const FaceIrisPlan& plan,
    ...
    try {
        ...
        std::vector<uint32_t> px(static_cast<size_t>(w) * h, 0);
        if (!ReadPixelsLocked(dst, w, h, px.data()))
            ReadPixelsSlow(dst, w, h, px.data());
...
void PaintFaceIrisForBuild(int faceId, const WeaponTint& t,
    ...
    for (size_t i = 0; i < plans.size() && i < _countof(kFaceIrisPaths); ++i) {
        ...
        if (!changed || WriteRuns(canvas, w, h, after.data()) <= 0)
            continue;
...
struct FaceIdGuard {
    ...
    ~FaceIdGuard() {
        if (avatar)
            avatar->m_avatarLook.nFace = original;
    }
};
...
bool NameOfAvatar(CAvatar* pAvatar, char* out, size_t cap) {
    ...
    if (len < 4)
        return false; // shortest legal MapleStory name
    out[len] = '\0';
    return true;
}
...
int CashWeaponOf(CAvatar* pAvatar) {
    ...
    int id = 0;
    __try {
        id = pAvatar->m_avatarLook.nWeaponStickerID;
    } __except (EXCEPTION_EXECUTE_HANDLER) {
        id = 0;
    }
...
struct TintScope {
    ...
    bool Lookup(int itemId, WeaponTint& out) const {
        if (preview && itemId == g_previewItemId) {
            out = g_preview;
            return true;
        }
...
TintScope ScopeForAvatar(CAvatar* pAvatar) {
    TintScope s;
    const bool isForced = (g_forcedScope > 0 && (g_forcedAvatar == nullptr || pAvatar == g_forcedAvatar));
    ...
    // Not every avatar is embedded in a CUser. Character-select, UI, and packet
    // construction paths create standalone/temporary CAvatars. Inferring a CUser
    // with `pAvatar - 0x88` and dereferencing its supposed name pointer therefore
    // raises an access violation on legitimate native objects. Until a remote
    // avatar is registered from a real CUser lifecycle hook, leave unknown avatars
    // untinted instead of probing memory around them.
    return s;
}
...
int CurrentActionOf(CAvatar* pAvatar) {
    int action = -1;
    __try {
        action = pAvatar->GetCurrentAction(nullptr);
    } __except (EXCEPTION_EXECUTE_HANDLER) {
        action = -1;
    }
    return action;
...
int WornItemsOf(CAvatar* pAvatar, int* out, int cap) {
    ...
    __try {
        ...
        if (sticker > 0 && sticker / 100000 == 17 && n < cap)
            out[n++] = sticker;
...
bool BeginItemEffSwap(CAvatar* pAvatar, int itemId) {
    ...
    if (!scope.any)
        return false;
    WeaponTint t;
    if (!scope.Lookup(EffectTintKeyFor(itemId), t) || t.IsIdentity())
        return false;
    try {
...
unsigned int WeaponTint_EffectKeyForAvatar(void* pAvatar, int itemId) {
    CAvatar* p = reinterpret_cast<CAvatar*>(pAvatar);
    if (!p || itemId <= 0)
        return 0;
    const TintScope scope = ScopeForAvatar(p);
    if (!scope.any)
        return 0;
    WeaponTint t;
    if (!scope.Lookup(EffectTintKeyFor(itemId), t) || t.IsIdentity())
...
WeaponTint WeaponTint_GetEffectiveFor(int itemId) {
    if (g_previewActive && itemId == g_previewItemId)
        return g_preview;
...
void WeaponTint_InstallPreviewFaceTint(void* pAvatar) {
    ...
    if (!avatar || avatar != g_prismPreviewAvatar)
        return;
    int hairId = 0, faceId = 0;
    if (!SehLookIdsOf(avatar, hairId, faceId) || faceId <= 0)
        return;
    const TintScope scope = ScopeForAvatar(avatar);
    WeaponTint tint;
    if (!scope.Lookup(kTintKey_Face, tint) || tint.IsIdentity())
        return;
...
void WeaponTint_RefreshLocalAvatar() {
    ...
    __try {
        *reinterpret_cast<void**>(reinterpret_cast<unsigned char*>(pAvatar) + 0x4E0) = nullptr;
    } __except (EXCEPTION_EXECUTE_HANDLER) {
        return;
    }
...
void WeaponTint_HandleSync(CInPacket* p) {
    ...
    auto readEntry = [&](TintMap& into) -> bool {
        ...
        if (t.hue < 0 || t.hue > kTintHueMax)
            t.hue = 0;
        if (itemId > 0 && !t.IsIdentity())
            into[itemId] = t;
        return true;
    };
    ...
    if (subtype == kResp_Snapshot) {
        ...
        for (int i = 0; i < count; ++i) {
            if (!readEntry(parsed))
                return; // truncated: keep what we had
        }
        std::lock_guard<std::mutex> lock(g_mtx);
        if (parsed != g_saved)
            g_syncDirty = true;
    ...
    if (subtype == kResp_RemoteTable) {
        ...
        for (int i = 0; i < players; ++i) {
            ...
            if (len < 1 || len > 13 || !p->CanRead(static_cast<size_t>(len) + 1))
                return;
            ...
            if (!m.empty())
                parsed.emplace(std::move(name), std::move(m));
        }
        std::lock_guard<std::mutex> lock(g_mtx);
        ...
        for (const auto& kv : g_remoteTints) {
            if (parsed.find(kv.first) == parsed.end())
                g_remoteDirty.push_back(kv.first);
        }
